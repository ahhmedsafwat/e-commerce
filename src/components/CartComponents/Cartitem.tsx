...
const CartItem = () => {
  ...
  return state.map((item) => (
    <div
      className="flex py-6 small:py-3 mx-6 small:mx-3 border-b last:border-none justify-between relative"
      key={item.prodcut.id}
    >
      <Link
        to={`../products/${item.prodcut.id}/${item.prodcut.title}`}
        className="block"
      >
        <div className="flex">
          ...
          <div className="ml-4 flex flex-col justify-between ">
            <div>
              ...
              <div className="font-satoshireguler small:text-xs">
                Size:
                <span className="text-gray-400 ml-[2px]">{item.size}</span>
              </div>
              <div className="font-satoshireguler small:text-xs">
                Rating:
                <span className="text-gray-400 ml-[2px]">
                  {item.prodcut.rating.rate}
                </span>
              </div>
            </div>
            <div className="mb-2 text-2xl font-satoshiBold small:text-xl">
              {item.prodcut.price}
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col items-end justify-between">
        ...
        <div className="flex w-16 py-1  bg-[#F0F0F0] items-center justify-around rounded-full large:absolute large:bottom-4">
          <span className="select-none">{item.ammount}</span>
        </div>
      </div>
    </div>
  ));
